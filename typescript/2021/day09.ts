  ...
  executePartOne(input: string[]): number {
    ...
    for (let row = 0; row < heightMap.rowCount; row++) {
      for (let column = 0; column < heightMap.columnCount; column++) {
        if (
          (row === 0 ||
            heightMap.getValue(row, column) <
              heightMap.getValue(row - 1, column)) &&
          (column === 0 ||
            heightMap.getValue(row, column) <
              heightMap.getValue(row, column - 1)) &&
...
function locationExistsInArray(
  basinItemArray: Location[],
  row: number,
  column: number,
) {
  return (
    basinItemArray.find((x) => x.row === row && x.column === column) !==
    undefined
  );
}
